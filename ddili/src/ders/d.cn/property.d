...
$(P
在学习 slice 的时候我们已经多次用到它。slice 的 $(C length) 属性返回了其包含元素的个数：
)
...
$(P
和 slice 的 $(C length) 属性一样，用户定义的属性也可以用于赋值：
)
...
$(P
之前我们通过定义属性为 $(C Rectangle) 添加了“第三个变量”。当然，除了属性，普通成员函数也可以实现我们需要的功能：
)
