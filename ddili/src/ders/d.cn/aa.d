...
$(P
关联数组是大多数现代高级编程语言所具有的功能。它们有像mini数据库那样执行得非常快的数据结构， 在很多程序里都用到了。
)

$(P
在 $(LINK2 /ders/d.cn/arrays.html, 数组一章) 中我们已经看到简单数组作为容器逐个存储元素，按索引访问它们。一个存储了星期的英文名称的数组可以这样定义：
)
...
$(P
hash表的高性能的代价是元素的存储是无序的。不像数组，hash表的元素不是逐个排列存储的。
)
...
$(P
另一方面，关联数组既存储键又存储元素值。虽然这种不同让关联数组使用更多内存，但它也允许它们使用 $(I sparse) 键值。例如，对于键 0 和 999 ，当只有两个元素要存储时，关联数组就只存储两个元素，而不像简单数组那样必须 1000 个。
)
...
$(P
随着每次的关联表会自动增长。例如，上面的操作结束后 $(C dayNumbers) 将有两个键值对。这可以通过打印整个表来验证：
)
...
$(P
每个键有且仅有一个值。因而，给一个存在的键赋值，表不增长，而相应键的值会变化：
)
...
$(LI $(IX .sizeof, associative array) $(C .sizeof) 数组大小$(I 参考) (它不影响表中键值对的个数，对全部关联数组来说值是一样的)。)

$(LI $(IX .get) $(C .get) 值存在就返回，否则是默认值。)
...
$(P
在 $(C grades["emre"]) 中通过学生姓名关联数组让它很容易访问到成绩。然而，在下面的代码中不能插入成绩，因为每个成绩将覆盖前一个：
)
