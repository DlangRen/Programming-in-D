...
$(DERS_BOLUMU $(IX 并发数据共享) $(IX 并发, 数据共享) 并发数据共享)
...
$(P
另外一种共享消息的方法是多个线程读写同一块数据。比如所有者线程在启动工作线程的同时向其传递了一个 $(C bool) 值的地址，工作线程可通过读取这个值来判断是否需要终止。或者所有者线程将同一个变量地址发给多个工作线程，这些工作线程可通过修改或读取这个变量来从其他线程获取信息。
)
...
$(P
D 语言的数据不会自动共享，默认情况下数据只能在其自己的线程中使用。这与其他编程语言有很大的区别。虽然每个线程都可以访问模块级的变量，但实际上它们访问的都是对应变量在自己线程中的副本：
)
...
$(P
由于每个线程都有一份单独的数据拷贝，$(C spawn()) 不允许以引用的形式传递线程内的值。比如下面这个程序，试图传递 $(C bool) 值的地址只会导致编译错误：
)
...
$(P
这样的话程序中只会有一 $(C globallyShared)，它会在所有线程间共享。在与像 C 和 C++ 这样默认自动数据共享的语言编写的库交互时，$(C __gshared) 是必须的。
)
...
$(P
$(I $(B 注：) 推荐使用消息传递向线程发送控制信号。)
)

$(P
$(IX immutable, 并发) $(IX immutable, concurrency) 而由于 $(C immutable) 变量无法被修改，它们可以直接共享。所以实际上 $(C immutable) 隐含了 $(C shared)：
)
...
$(P
注意 $(C i) 的生命期即为 $(C main()) 函数作用域，所以为了防止出现错误应确保 $(C main()) 函数在工作线程终止后才返回。所以我们调用了 $(C core.thread.thread_joinAll) 函数阻塞主线程来等待子线程执行完毕。
)
...
$(P
经过这种情况的执行后 $(C i) 和 $(C j) 的值最后都变成了 1。此处不可能会再有其他的值了。
)
...
$(H5 $(IX synchronized) 使用 $(C synchronized) 避免竞态条件)

$(P
程序出错的原因是多个线程访问同一块可变数据（并且其中至少有一条线程修改了数据）。一种解决方案是使用 $(C synchronized) 关键字标记公共代码以消除竞态条件。经过下面的修改程序就能正确执行了：
)
...
$(P
$(IX lock) $(C synchronized) 会在后台创建一个锁，同一时间只有一个线程能持有这个锁。只有持有锁的那个线程才可以执行，其他线程都需要等待持有锁的线程执行完成并释放 $(C synchronized) 锁。由于同一时间只有一个执行 $(I synchronized) 代码的线程，我们就可以安全的进行交换。在同步块执行后 $(C i) 与 $(C j) 只会有种情况：要么是“1 和 2”，要么是“2 和 1”。
)

$(P
$(I $(B 注：)等待锁是一个相对昂贵的操作，它可能会显著降低程序的执行速度。幸运的是大多数程序可以使用$(I 原子操作)替代 $(C synchronized) 块。一会我们会介绍它。)
)
...
$(P
$(I $(B 注：)如果将上方的等式换成自增或自检（比如 $(C ++(*value)) 和 $(C &#8209;&#8209;(*value))），编译器会警告：对 $(C shared) 变量执行的读取-修改-写入操作已被弃用。)
)

$(P
然而直接使用 $(C synchronized) 并不能起到我们预期的效果，因为两个代码块的匿名锁是相互独立的。所以这两块代码还是会同时访问那个变量：
)
...
$(P
线程数相同且加减次数相同，可能有人就会认为 $(C number) 的最终结果是 0。然而这个程序几乎不会得出这个结果：
)
...
$(P
D 中没有专门的 lock 类型，任何类型都可以作为 $(C synchronized) 锁。下面这个程序定义了一个空的 $(C Lock) 类作为锁：
)
...
$(P
这次两个 $(C synchronized) 块使用了同一个锁，同一时间它们中只有一个可以执行。这样最终结果就与我们预期的 0 一样了：
)
...
$(P
类也可以定义为 $(C synchronized)。这样同一时间只能有一个线程调用类的示例对象的非静态成员函数：
)
...
$(P
$(IX 死锁) 我们会用一个例子来解释程序为什么出错。示例中有一个线程想要将账户 A 中的资金转到账户 B，而另一个线程想要将账户 B 中的资金转到账户 A。每个线程都会先锁住各自的 $(C from) 对象，然后再尝试去锁 $(C to) 对象。由于代表 A 和 B 的 $(C from) 对象已分别被两个线程锁住，它们将无法获取另一个线程的 $(C to) 对象（即刚刚被锁住的 B 和 A）。这个现象就是$(I 死锁)。
)
...
$(H5 $(IX shared static this) $(IX static this, shared) $(IX shared static ~this) $(IX static ~this, shared) $(IX this, shared static) $(IX ~this, shared static) $(IX module constructor, shared) 使用 $(C shared static this()) 单次初始化和使用 $(C shared static ~this()) 单次析构)
...
$(P
对于指定了 $(C share) 的模块变量来说，重复初始化有可能导致并发中的竞态条件而造成程序出错。（这也适用于 $(C immutable) 因为它已隐含 $(C shared)。）解决方案是使用 $(C shared static this()) 块，它只会在程序中执行一次：
)

---
int a;              // 每个线程拥有一个独立副本
immutable int b;    // 被所有线程共享
...
$(P
$(C shared static ~this()) 也一样，每个程序只会执行一次来释放资源。
)
...
$(P
D 语言中的原子操作都在 $(C core.atomic) 模块中。本章我们只会接触到其中两种：
)

$(H6 $(IX atomicOp, core.atomic) $(C atomicOp))

$(P
这个函数会将它的模版参数应用到两个函数参数上。它的模版参数必须是一个$(I 二元运算符)，比如 $(STRING “+”)，$(STRING “+=”)。
)
...
$(P
如果只是一个二元运算的话，我们没必要使用会影响效率的 $(C synchronized) 块。下面是使用 $(C atomicOp) 的等价 $(C incrementer()) 和 $(C decrementer()) 函数。注意现在不再需要 $(C Lock) 类了：
)
...
$(P
如果 $(C cas()) 开始执行后变量的值还是等于 $(C currentValue)，则说明自当前线程读入后这个变量没有被其他线程修改。这样的话 $(C cas()) 会将 $(C newValue) 赋给这个变量并返回 $(C true)。如果 $(C cas()) 发现变量的值不再等于 $(C currentValue) 了，它将直接返回 $(C false)，不再修改变量的值。
)
...
$(P
$(C core.sync) 中的模块包含了一些古老的并发工具，感兴趣的话可以去了解一下：
)
...
$(LI 若要使用并发，优先选择上一章的$(I 并发消息传递)模型。)
...
macros:
        ...
        DESCRIPTION=执行多条线程并在线程间共享数据

        KEYWORDS=D 编程语言教程 线程并发 数据共享
