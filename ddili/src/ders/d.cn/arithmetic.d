...
$(P
Integer types are the types that can have only whole values like -2, 0, 10, etc. These types cannot have fractional parts, as in 2.5. All of the integer types that we have seen in the $(LINK2 /ders/d.en/types.html, Fundamental Types chapter) are the following:
)
...
$(P
As there aren't many concepts that can be represented by just two states, bit is not a very useful type. It can only be useful for concepts with two states like heads or tails or whether a light switch is on or off.
)
...
$(P
The values to which these eight states correspond to are defined by conventions. The following table shows these values for the signed and unsigned representations of 3 bits:
)
...
$(P
I will take advantage of the $(C .min) and $(C .max) properties below, which we have seen in the $(LINK2 /ders/d.en/types.html, Fundamental Types chapter). These properties provide the minimum and maximum values that an integer type can have.
)
...
void main() {
    ...
    writeln("Result: ", first $(HILITE +) second);
    writeln("With a constant expression: ", 1000 $(HILITE +) second);
}
---
...
$(P
If the result is greater than the sum of the two expressions, it again overflows and becomes a value that is less than both of the expressions:
)
...
$(P
$(I $(B Note:) Unless there is a strong reason, always use the regular increment operator (which is sometimes called the pre-increment operator).)
)
...
$(P
$(I $(B Note:) Unless there is a strong reason, always use the regular decrement operator (which is sometimes called the pre-decrement operator).)
)
...
$(PROBLEM
Write a program that prints the values from 1 to 10, each on a separate line, with the exception of value 7. Do not use repeated lines like the following:
...
Macros:
        SUBTITLE=Integers and Arithmetic Operations
