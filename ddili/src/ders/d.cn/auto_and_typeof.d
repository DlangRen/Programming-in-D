...
$(P
显然，像上面那种实际类型已知的情况下，$(C typeof) 并不是必需的。相反，你通常会在更复杂的情况下才使用它，比如当你希望你的变量类型同它处代码中的可以改变的类型保持一致的时候。该关键字在 $(LINK2 /ders/d.en/templates.html, 模板) 和 $(LINK2 /ders/d.en/mixin.html, 混入) 中特别有用，此二者将在后面的章节中介绍。
)
