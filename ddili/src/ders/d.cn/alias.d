...
$(P
要解决这个冲突，一个便利的方式就是给这些名字定义一个 alias：
)
...
$(P
$(C alias) 也可以用于其他的名字，下面的代码为变量定义了一个新名字：
)
...
$(P
因为 class 中的成员变量和属性都定义成了 $(C double)，用户也就必须使用 $(C double)：
)
...
$(P
我们来比较一下另外一种设计，把 $(C 重量) 定义为一个 alias：
)
...
$(P
名字隐藏同时也会影响成员变量，当然，$(C alias) 也可以引入这些名字：
)
...
$(P
虽然一个是成员变量，一个是成员函数，子类重的名字 $(C city) 也会隐藏父类重的名字 $(C city)：
)
