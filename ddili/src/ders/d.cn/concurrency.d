...
$(P
$(IX thread) $(B 线程)：操作系统执行程序的工作单元叫做$(I 线程)。D 语言程序在操作系统指定的线程上执行 $(C main()) 函数。通常情况下程序的所有操作都将在这个线程中完成。程序也可以自由的创建线程以实现在同一时间执行多个任务的功能。实际上上一章我们学习的 task 就是基于线程的，只不过这些线程是由 $(C std.parallelism) 自动启动的。
)
...
$(P
上面这个看似简单的操作实际上包含三个步骤：读取变量的值、将其加一、将结果写回变量所在的内存。线程可能会暂停在这三步中的任何一步上，经过一段时间后才会继续。
)
...
$(P
$(IX worker) $(B Worker)：被所有者启动的线叫做工人（worker）。
)

$(H5 $(IX spawn) 启动线程)

$(P
$(C spawn()) 需要一个函数指针，新线程将会从指定的函数启动。函数中进行的所有操作包括函数调用都将在新线程中执行。与 $(LINK2 /ders/d.cn/parallelism.html, $(C task())) 启动的线程相比，$(C spawn()) 启动的线程与之最大的不同在于 $(C spawn()) 允许线程间消息传递。
)
...
$(P
程序在所有线程执行完毕后才会退出。从上面的输出中我们可以看到，$(C main()) 函数打印 “main is done.”退出后 $(C worker()) 还在继续执行。
)
...
$(P
也可以在一次消息中发送多个值，这些值都会称为这次消息的一部分。下面这个消息就是由三个部分组成：
)
...
$(P
所有者无法捕获由工作线程抛出的异常。一种解决方案是在工作线程中捕获潜在的由于接收信息引发的异常。我们之后会见到的。
)
...
$(P
本例中的所有消息都是从工作线程向线程所有者传递的。当然在许多程序中消息传递不止这么简答。
)
...
$(P
Lambda 函数和定了 $(C opCall()) 成员函数的对象都可以传递给 $(C receive()) 作为消息处理器。下面这个工作线程使用 lambda 函数处理消息。程序还定义了一个 $(C Exit) 类型来通知线程退出。相对于使用像 -1 这样的任意值，用一个特定的类型来传递特定的消息会让程序更易读。
)

$(P
有三个匿名函数被传递给了 $(C receive()) 来作为消息处理器。它们的花括号会被高亮：
)
...
$(P
可能经过一段时间后就不需要继续等待消息了。消息的发送者可能正在忙碌或因异常终止。$(C receiveTimeout()) 可以防止出现无限等待消息这样的情况。
)
...
$(P
上面的线程所有者等待消息 600 毫秒。如果消息超时它还会继续执行其他操作：
)
...
$(P
工作线程可以做的就是手动捕获异常并将其作为特殊的错误信息发送给所有者。下面这个程序就把出错的原因封装在 $(C CalculationFailure) 消息中传递回去。除此之外，这个程序还是用了特殊的消息类型来通知工作线程退出：
)
...
$(P
如果所有者线程已被终止，工作线程在接收消息时就会抛出这个异常。下方程序中的中间层的线程所有者在发送两条消息后就立即退出。这会导致工作线程抛出 $(C OwnerTerminated) 异常：
)
...
$(LI $(IX OnCrowding) $(C OnCrowding.block)：阻塞发送者直到收件箱中出现空闲空间。)
...
$(P
$(C setMaxMailboxSize()) 将收件箱大小限制为 1000. $(C OnCrowding.block) 会阻塞消息发送者的线程直到收件箱中出现空闲空间。
)
...
$(P
下面这三个函数定义了一个接口。通过这个接口每个线程都可以访问一个那个公共关联数组：
)
