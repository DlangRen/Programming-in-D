...
$(P
As its name suggests, this exception is inherited from $(C Error), and as you may remember from the $(LINK2 /ders/d.en/exceptions.html, Exceptions chapter), exceptions that are inherited from $(C Error) must never be caught. It is important for the program to be terminated right away instead of continuing under invalid assumptions.
)
...
$(SHELL
core.exception.$(HILITE AssertError)@deneme(3): Assertion failure
)

$(P
The part after the $(C @) character in the message indicates the source file and the line number of the $(C assert) check that failed. According to the output above, the $(C assert) that failed is on line 3 of file $(C deneme.d).
)
...
$(SHELL
core.exception.AssertError@deneme.d(3): $(HILITE Age cannot be negative.)
)
...
$(P
The function is guaranteed to work with the only two commands that it knows about. ($(I $(B Note:) An alternative choice here would be to use a $(LINK2 /ders/d.en/switch_case.html, $(C final switch) statement).))
)
...
$(P
We have seen that expressions produce values or make side effects. $(C assert) checks do not have values nor they $(I should) have any side effects.
)
...
$(P
This would allow programs run faster by not evaluating potentially slow logical expressions of the $(C assert) checks.
)
