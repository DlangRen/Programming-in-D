...
$(P
Unions allow more than one member share the same memory area. They are a low-level feature inherited from the C programming language.
)
...
$(P
The following is a simple discriminated union type that supports only two types: $(C int) and $(C double). In addition to a $(C union) to store the data, it maintains a $(LINK2 /ders/d.en/object.html, $(C TypeInfo)) member to know which one of the two $(C union) members is valid.
)
