...
$(P
在展示 $(C setMaxMailboxSize()) 示例之前，我们先来创建一个消息数量会持续增长的邮箱。下面这个工作线程会不停地向主线程发送消息，但主线程处理消息的速度就没有工作线程这么快了，每条消息主线程都会花费一点时间来处理：
)
...
$(P
因为消费者处理消息的速度远低于生产者产生消息的速度，程序的内存占用会持续增长。为了防止出现这种情况，所有者线程会在启动工作线程之前限制其邮箱的大小：
)
...
$(P
$(C setMaxMailboxSize()) 将邮箱大小限制为 1000。$(C OnCrowding.block) 会使得发送者线程等待，一直到邮箱中有足够空间为止。
)
...
$(P
之前我们看到的程序都很简单，所以在线程间传递线程 ID 还是比较方便的。一旦线程数增加，它将会大大增加程序的复杂度。为了降低复杂度，可以为线程分配一个命名——所有线程都可以通过名字访问该线程。
)
...
$(LI 如果线程间相互独立，推荐使用上一章的 $(I 并行)。只有在线程间有相互依赖的操作时才考虑 $(I 并发)。)

$(LI 基于数据共享的并发难以正确实现，因此推荐使用本章讲解的基于消息传递的并发。)

$(LI $(C spawn()) 和 $(C spawnLinked()) 用于启动线程。)

$(LI $(C thisTid) 为当前线程的 ID。)

$(LI $(C ownerTid) 为当前线程的所有者的 ID。)
