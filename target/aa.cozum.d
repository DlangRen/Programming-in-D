...
$(LI
特性 $(C .keys) 返回的是一个分片(即动态数组)，其中包含关联数组的所有键。采用迭代此分片，并对每个键调用 $(C .remove) 函数来移除元素的方法，最后会得到一个空的关联数组：
...
$(LI
目标是存储每个学生的多个成绩。由于多个成绩可以用一个动态数组存储，从 $(C string) 到 $(C int[]) 映射的关联数组将能用在此处。成绩能附加到存储在关联数组中的动态数组上：
