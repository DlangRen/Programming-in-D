...
$(P
$(IX foreach, parallel) $(C parallel()) 以并行的方式访问 range 中的元素。它可以在 $(C foreach) 循环中起到非常大的作用。只需要导入 $(C std.parallelism) 模块并将上面代码中的 $(C students) 换为 $(C parallel(students)) 即可充分利用系统中全部核心的运算能力。
)
...
$(P
只有在确定在每一次迭代中对元素的操作相互独立后，程序员才能调用 $(C parallel()) 来实现并行。例如，如果输出信息的顺序非常重要，那么上面程序中的 $(C parallel()) 调用将会使程序出错。支持线程的编程模型依赖线程的$(I 并发)调用。并发性是下一章的话题。
)
...
$(P
虽然将线程分派给核心的开销极小。虽然将线程分派给核心的开销极小。但在某些情况下这个开销会显得极其昂贵，尤其是在每次循环的操作耗时都非常短的时候。在这种情况下，让一个线程去执行多个迭代反而会使处理速度加快。工作单元大小决定了线程在其迭代中要处理元素的个数。
)
...
$(P
程序中以并行的方式执行的操作叫$(I 任务)。标准库中的 $(C std.parallelism.Task) 表示的就是我们所说的任务。
)
...
$(P
$(IX task) $(IX executeInNewThread) $(IX yieldForce) 如果要在处理范围中元素之外的地方使用 task，程序员需要按照以下三步手动处理任务：使用 $(C task()) 创建 task，使用 $(C executeInNewThread()) 启动 task，使用 $(C yieldForce()) 等待 task 完成。在下方程序的注释中有对这三个函数详细的解释。
)
...
$(P
这种方法并不会产生不同类型的 $(C Task) 模版实例，这样你就可以把它们放在同一个数组里：
)
...
$(P
为了防止将输入的可能为延迟取值（fully-lazy）的范围转换为即时取值（fully-eager）的 range，它会$(I 轮)的方式进行迭代。每轮在缓冲区中载入一定数量的元素用于并行迭代。只有当上一轮缓冲的元素被 $(C popFront()) 消耗完后，它才会开始为下一轮迭代缓冲元素。
)
...
$(P
$(C reduce()) 也是一个经常在函数式编程中使用的高阶函数。和 $(C map()) 一样，它也可以接收一个或多个函数作为模版实参。除了用于接收函数的模版形参，它需要传入一个运算结果的初始值和一个范围。$(C reduce()) 将传入的函数作为运算方法对每个元素进行计算并按照合并到结果中。$(C reduce()) 将传入的函数作为运算方法对每个元素进行计算并按照合并到结果中。
)
...
$(P
下面来看一个示例，将 $(C reduce()) 与一个人为减慢的函数一起使用作：
)
