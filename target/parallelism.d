...
$(P
在这些场景的背后，源自 $(C std.parallelism) 模块的所有并行算法使用的任务对象都是 $(C TaskPool) 容器里的元素。一般情况下，所有算法都会使用同一个名为 $(C taskPool) 的容器对象。
)

$(P
$(C taskPool) 需要容纳的任务总数取决于程序运行的具体环境。因此，通常不需要创建其他的 $(C TaskPool) 对象。即便如此，也可以在需要的时候显示创建和使用 $(C TaskPool) 对象。
)

$(P
$(C TaskPool) 构造函数有一个参数。通过指定这个参数就可以控制之后并行执行的线程个数。默认的线程数量比系统内核数少 1。本章介绍的所有功能都可以通过使用一个单独的 $(C TaskPool) 来调用。
)
